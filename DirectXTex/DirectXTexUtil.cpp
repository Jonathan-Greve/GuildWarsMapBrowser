...
static_assert(XBOX_DXGI_FORMAT_R10G10B10_7E3_A2_FLOAT == DXGI_FORMAT_R10G10B10_7E3_A2_FLOAT,
              "Xbox mismatch detected");
static_assert(XBOX_DXGI_FORMAT_R10G10B10_6E4_A2_FLOAT == DXGI_FORMAT_R10G10B10_6E4_A2_FLOAT,
              "Xbox mismatch detected");
static_assert(XBOX_DXGI_FORMAT_D16_UNORM_S8_UINT == DXGI_FORMAT_D16_UNORM_S8_UINT, "Xbox mismatch detected");
static_assert(XBOX_DXGI_FORMAT_R16_UNORM_X8_TYPELESS == DXGI_FORMAT_R16_UNORM_X8_TYPELESS,
              "Xbox mismatch detected");
static_assert(XBOX_DXGI_FORMAT_X16_TYPELESS_G8_UINT == DXGI_FORMAT_X16_TYPELESS_G8_UINT,
              "Xbox mismatch detected");
static_assert(XBOX_DXGI_FORMAT_R10G10B10_SNORM_A2_UNORM == DXGI_FORMAT_R10G10B10_SNORM_A2_UNORM,
              "Xbox mismatch detected");
...
BOOL WINAPI InitializeWICFactory(PINIT_ONCE, PVOID, PVOID* ifactory) noexcept
{
    ...
    HRESULT hr = CoCreateInstance(CLSID_WICImagingFactory2, nullptr, CLSCTX_INPROC_SERVER,
                                  __uuidof(IWICImagingFactory2), ifactory);
    ...
    else
    {
        ...
        hr = CoCreateInstance(CLSID_WICImagingFactory1, nullptr, CLSCTX_INPROC_SERVER,
                              __uuidof(IWICImagingFactory), ifactory);
...
_Use_decl_annotations_ bool DirectX::Internal::DXGIToWIC(DXGI_FORMAT format, GUID& guid,
                                                         bool ignoreRGBvsBGR) noexcept
{
    switch (format)
    {
...
TEX_FILTER_FLAGS DirectX::Internal::CheckWICColorSpace(_In_ const GUID& sourceGUID,
                                                       _In_ const GUID& targetGUID) noexcept
{
...
IWICImagingFactory* DirectX::GetWICFactory(bool& iswic2) noexcept
{
    ...
    if (! InitOnceExecuteOnce(&s_initOnce, InitializeWICFactory, nullptr,
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ bool DirectX::IsPlanar(DXGI_FORMAT fmt) noexcept
{
    switch (static_cast<int>(fmt))
    {
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::ComputePitch(DXGI_FORMAT fmt, size_t width, size_t height,
                                                     ...
                                                     CP_FLAGS flags) noexcept
{
    ...
    default:
        assert(! IsCompressed(fmt) && ! IsPacked(fmt) && ! IsPlanar(fmt));
        {
            ...
            if (flags &
                (CP_FLAGS_LEGACY_DWORD | CP_FLAGS_PARAGRAPH | CP_FLAGS_YMM | CP_FLAGS_ZMM | CP_FLAGS_PAGE4K))
            {
...
_Use_decl_annotations_ size_t TexMetadata::ComputeIndex(size_t mip, size_t item, size_t slice) const noexcept
{
    if (mip >= mipLevels)
        return size_t(-1);
