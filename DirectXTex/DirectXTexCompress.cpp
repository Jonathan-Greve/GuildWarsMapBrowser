...
constexpr uint32_t GetBCFlags(_In_ TEX_COMPRESS_FLAGS compress) noexcept
{
    ...
    static_assert(static_cast<int>(TEX_COMPRESS_A_DITHER) == static_cast<int>(BC_FLAGS_DITHER_A),
                  "TEX_COMPRESS_* flags should match BC_FLAGS_*");
    static_assert(static_cast<int>(TEX_COMPRESS_DITHER) ==
                    static_cast<int>(BC_FLAGS_DITHER_RGB | BC_FLAGS_DITHER_A),
                  "TEX_COMPRESS_* flags should match BC_FLAGS_*");
    ...
    static_assert(static_cast<int>(TEX_COMPRESS_BC7_QUICK) == static_cast<int>(BC_FLAGS_FORCE_BC7_MODE6),
                  "TEX_COMPRESS_* flags should match BC_FLAGS_*");
    return (compress &
            (BC_FLAGS_DITHER_RGB | BC_FLAGS_DITHER_A | BC_FLAGS_UNIFORM | BC_FLAGS_USE_3SUBSETS |
             BC_FLAGS_FORCE_BC7_MODE6));
}

constexpr TEX_FILTER_FLAGS GetSRGBFlags(_In_ TEX_COMPRESS_FLAGS compress) noexcept
{
    static_assert(TEX_FILTER_SRGB_IN == 0x1000000,
                  "TEX_FILTER_SRGB flag values don't match TEX_FILTER_SRGB_MASK");
...
inline bool DetermineEncoderSettings(_In_ DXGI_FORMAT format, _Out_ BC_ENCODE& pfEncode,
                                     _Out_ size_t& blocksize, _Out_ TEX_FILTER_FLAGS& cflags) noexcept
{
    switch (format)
    {
...
//-------------------------------------------------------------------------------------
HRESULT CompressBC(const Image& image, const Image& result, uint32_t bcflags, TEX_FILTER_FLAGS srgb,
        ...
        for (size_t count = 0; (count < result.rowPitch) && (w < image.width); count += blocksize, w += 4)
        {
            ...
            if (ph > 1)
            {
                ...
                if (ph > 2)
                {
                    ...
                    {
                        bytesToRead =
                          std::min<size_t>(rowPitch, static_cast<size_t>(bytesLeft) - rowPitch * 3);
...
//-------------------------------------------------------------------------------------
#ifdef _OPENMP
HRESULT CompressBC_Parallel(const Image& image, const Image& result, uint32_t bcflags, TEX_FILTER_FLAGS srgb,
                            float threshold) noexcept
{
    if (! image.pixels || ! result.pixels)
        return E_POINTER;
    ...
    const size_t nBlocks =
      std::max<size_t>(1, (image.width + 3) / 4) * std::max<size_t>(1, (image.height + 3) / 4);
...
// Compression
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Compress(const Image& srcImage, DXGI_FORMAT format,
                                                 ...
                                                 ScratchImage& image) noexcept
{
    ...
    if (IsTypeless(format) || IsTypeless(srcImage.format) || IsPlanar(srcImage.format) ||
...
_Use_decl_annotations_ HRESULT DirectX::Compress(const Image* srcImages, size_t nimages,
                                                 ...
                                                 ScratchImage& cImages) noexcept
{
    ...
    if (IsTypeless(format) || IsTypeless(metadata.format) || IsPlanar(metadata.format) ||
    ...
    for (size_t index = 0; index < nimages; ++index)
    {
        ...
        if ((compress & TEX_COMPRESS_PARALLEL))
        {
            ...
            if (compress & TEX_COMPRESS_PARALLEL)
            {
                hr = CompressBC_Parallel(src, dest[index], GetBCFlags(compress), GetSRGBFlags(compress),
                                         threshold);
                if (FAILED(hr))
                {
                    cImages.Release();
...
// Decompression
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Decompress(const Image& cImage, DXGI_FORMAT format,
                                                   ScratchImage& image) noexcept
{
...
_Use_decl_annotations_ HRESULT DirectX::Decompress(const Image* cImages, size_t nimages,
                                                   const TexMetadata& metadata, DXGI_FORMAT format,
