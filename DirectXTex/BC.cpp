...
inline uint16_t Encode565(_In_ const HDRColorA* pColor) noexcept
{
    ...
    w = static_cast<uint16_t>((static_cast<int32_t>(Color.r * 31.0f + 0.5f) << 11) |
...
//-------------------------------------------------------------------------------------
void OptimizeRGB(_Out_ HDRColorA* pX, _Out_ HDRColorA* pY,
                 _In_reads_(NUM_PIXELS_PER_BLOCK) const HDRColorA* pPoints, uint32_t cSteps,
                 uint32_t flags) noexcept
{
    ...
    for (size_t iIteration = 0; iIteration < 8; iIteration++)
    {
        ...
        for (size_t iPoint = 0; iPoint < NUM_PIXELS_PER_BLOCK; iPoint++)
        {
            const float fDot = (pPoints[iPoint].r - X.r) * Dir.r + (pPoints[iPoint].g - X.g) * Dir.g +
...
//-------------------------------------------------------------------------------------
inline void DecodeBC1(_Out_writes_(NUM_PIXELS_PER_BLOCK) XMVECTOR* pColor, _In_ const D3DX_BC1* pBC,
                      bool isbc1) noexcept
{
    assert(pColor && pBC);
...
//-------------------------------------------------------------------------------------
void EncodeBC1(_Out_ D3DX_BC1* pBC, _In_reads_(NUM_PIXELS_PER_BLOCK) const HDRColorA* pColor, bool bColorKey,
               float threshold, uint32_t flags) noexcept
{
    assert(pBC && pColor);
    static_assert(sizeof(D3DX_BC1) == 8, "D3DX_BC1 should be 8 bytes");
    ...
    const float fScale =
      (wColorA != wColorB) ? (fSteps / (Dir.r * Dir.r + Dir.g * Dir.g + Dir.b * Dir.b)) : 0.0f;
    ...
    for (i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
    {
        ...
        else
        {
            ...
            const float fDot =
              (Clr.r - Step[0].r) * Dir.r + (Clr.g - Step[0].g) * Dir.g + (Clr.b - Step[0].b) * Dir.b;
...
_Use_decl_annotations_ void DirectX::D3DXEncodeBC1(uint8_t* pBC, const XMVECTOR* pColor, float threshold,
                                                   uint32_t flags) noexcept
{
    assert(pBC && pColor);
...
_Use_decl_annotations_ void DirectX::D3DXEncodeBC2(uint8_t* pBC, const XMVECTOR* pColor,
                                                   uint32_t flags) noexcept
{
    assert(pBC && pColor);
    static_assert(sizeof(D3DX_BC2) == 16, "D3DX_BC2 should be 16 bytes");
...
_Use_decl_annotations_ void DirectX::D3DXDecodeBC3(XMVECTOR* pColor, const uint8_t* pBC) noexcept
{
    ...
    uint32_t dw =
      uint32_t(pBC3->bitmap[0]) | uint32_t(pBC3->bitmap[1] << 8) | uint32_t(pBC3->bitmap[2] << 16);
...
_Use_decl_annotations_ void DirectX::D3DXEncodeBC3(uint8_t* pBC, const XMVECTOR* pColor,
                                                   uint32_t flags) noexcept
{
    assert(pBC && pColor);
    static_assert(sizeof(D3DX_BC3) == 16, "D3DX_BC3 should be 16 bytes");
