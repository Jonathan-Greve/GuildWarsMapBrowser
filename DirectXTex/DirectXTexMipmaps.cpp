...
namespace
{
constexpr bool ispow2(_In_ size_t x) noexcept { return ((x != 0) && ! (x & (x - 1))); }
...
#ifdef _WIN32
HRESULT EnsureWicBitmapPixelFormat(_In_ IWICImagingFactory* pWIC, _In_ IWICBitmap* src,
    ...
    *dest = nullptr;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = converter->Initialize(src, desiredPixelFormat, GetWICDither(filter), nullptr, 0,
...
HRESULT CalculateAlphaCoverage(const Image& srcImage, float alphaReference, float alphaScale,
                               float& coverage) noexcept
{
    ...
    {
        if (! LoadScanlineLinear(row0.get(), srcImage.width, pSrcRow0, srcImage.rowPitch, srcImage.format,
                                 TEX_FILTER_DEFAULT))
        {
            return E_FAIL;
        }
        ...
        if (! LoadScanlineLinear(row1.get(), srcImage.width, pSrcRow1, srcImage.rowPitch, srcImage.format,
                                 TEX_FILTER_DEFAULT))
        {
            return E_FAIL;
        }
...
HRESULT EstimateAlphaScaleForCoverage(const Image& srcImage, float alphaReference, float targetCoverage,
...
_Use_decl_annotations_ bool DirectX::Internal::CalculateMipLevels(size_t width, size_t height,
...
_Use_decl_annotations_ bool DirectX::Internal::CalculateMipLevels3D(size_t width, size_t height, size_t depth,
                                                                    size_t& mipLevels) noexcept
{
    if (mipLevels > 1)
    {
...
_Use_decl_annotations_ HRESULT DirectX::Internal::ResizeSeparateColorAndAlpha(
  IWICImagingFactory* pWIC, bool iswic2, IWICBitmap* original, size_t newWidth, size_t newHeight,
  TEX_FILTER_FLAGS filter, const Image* img) noexcept
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr =
              EnsureWicBitmapPixelFormat(pWIC, original, filter, colorPixelFormat, converted.GetAddressOf());
            if (SUCCEEDED(hr))
            {
                hr = colorScaler->Initialize(converted.Get(), static_cast<UINT>(newWidth),
                                             static_cast<UINT>(newHeight), interpolationMode);
            }
        }
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr =
              pWIC->CreateBitmapFromSource(colorScaler.Get(), WICBitmapCacheOnDemand, resized.GetAddressOf());
            if (SUCCEEDED(hr))
            {
                hr = EnsureWicBitmapPixelFormat(pWIC, resized.Get(), filter, colorPixelFormat,
                                                resizedColor.GetAddressOf());
            }
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            ComPtr<IWICBitmap> converted;
            hr = EnsureWicBitmapPixelFormat(pWIC, original, filter, colorWithAlphaPixelFormat,
                                            converted.GetAddressOf());
            if (SUCCEEDED(hr))
            {
                hr = colorWithAlphaScaler->Initialize(converted.Get(), static_cast<UINT>(newWidth),
                                                      static_cast<UINT>(newHeight), interpolationMode);
            }
        }
        ...
        {
            ComPtr<IWICBitmap> resized;
            hr = pWIC->CreateBitmapFromSource(colorWithAlphaScaler.Get(), WICBitmapCacheOnDemand,
                                              resized.GetAddressOf());
            if (SUCCEEDED(hr))
            {
                hr = EnsureWicBitmapPixelFormat(pWIC, resized.Get(), filter, colorWithAlphaPixelFormat,
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            for (size_t j = 0; SUCCEEDED(hr) && j < newHeight; j++)
            {
                ...
                {
                    size_t colorWithAlphaIndex =
                      (j * colorWithAlphaStride) + (i * colorWithAlphaBytesPerPixel);
...
#pragma warning(suppress : 26014 6386) // No overflow possible here
                        memcpy_s(colorWithAlphaData + colorWithAlphaIndex, colorWithAlphaBytesPerPixel,
                                 colorData + colorIndex, colorBytesInPixel);
                    }
                }
            }
        }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = EnsureWicBitmapPixelFormat(pWIC, resizedColorWithAlpha.Get(), filter, desiredPixelFormat,
                                        wicBitmap.GetAddressOf());
        if (SUCCEEDED(hr))
        {
            hr = wicBitmap->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch),
                                       static_cast<UINT>(img->slicePitch), img->pixels);
        }
    }
...
HRESULT GenerateMipMapsUsingWIC(_In_ const Image& baseImage, _In_ TEX_FILTER_FLAGS filter, _In_ size_t levels,
                                _In_ const WICPixelFormatGUID& pfGUID, _In_ const ScratchImage& mipChain,
                                _In_ size_t item) noexcept
{
    ...
    HRESULT hr = pWIC->CreateBitmapFromMemory(
      static_cast<UINT>(width), static_cast<UINT>(height), pfGUID, static_cast<UINT>(baseImage.rowPitch),
      static_cast<UINT>(baseImage.slicePitch), baseImage.pixels, source.GetAddressOf());
    if (FAILED(hr))
        return hr;
    ...
    for (size_t level = 1; level < levels; ++level)
    {
        ...
        else
        {
            ...
            hr = scaler->Initialize(source.Get(), static_cast<UINT>(width), static_cast<UINT>(height),
                                    GetWICInterp(filter));
            if (FAILED(hr))
                return hr;
            ...
            {
                hr = scaler->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch),
                                        static_cast<UINT>(img->slicePitch), img->pixels);
                if (FAILED(hr))
                    return hr;
            }
            else
            {
                ...
                hr = FC->Initialize(scaler.Get(), pfGUID, GetWICDither(filter), nullptr, 0,
                                    WICBitmapPaletteTypeMedianCut);
                if (FAILED(hr))
                    return hr;

                hr = FC->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch),
                                    static_cast<UINT>(img->slicePitch), img->pixels);
                if (FAILED(hr))
                    return hr;
            }
...
//-------------------------------------------------------------------------------------
HRESULT Setup2DMips(_In_reads_(nimages) const Image* baseImages, _In_ size_t nimages,
                    _In_ const TexMetadata& mdata, _Out_ ScratchImage& mipChain) noexcept
{
...
//--- 2D Box Filter ---
HRESULT Generate2DMipsBoxFilter(size_t levels, TEX_FILTER_FLAGS filter, const ScratchImage& mipChain,
                                size_t item) noexcept
{
...
//--- 2D Linear Filter ---
HRESULT Generate2DMipsLinearFilter(size_t levels, TEX_FILTER_FLAGS filter, const ScratchImage& mipChain,
                                   size_t item) noexcept
{
    using namespace DirectX::Filters;
        ...
        for (size_t y = 0; y < nheight; ++y)
        {
            ...
            if (toY.u0 != u0)
            {
                if (toY.u0 != u1)
                {
                    ...
                    if (! LoadScanlineLinear(row0, width, pSrc + (rowPitch * u0), rowPitch, src->format,
                                             filter))
                        return E_FAIL;
                }
                else
...
HRESULT Generate2DMipsCubicFilter(size_t levels, TEX_FILTER_FLAGS filter, const ScratchImage& mipChain,
                                  size_t item) noexcept
{
    using namespace DirectX::Filters;
    ...
    for (size_t level = 1; level < levels; ++level)
    {
        ...
        const size_t nwidth = (width > 1) ? (width >> 1) : 1;
        CreateCubicFilter(width, nwidth, (filter & TEX_FILTER_WRAP_U) != 0,
                          (filter & TEX_FILTER_MIRROR_U) != 0, cfX);

        const size_t nheight = (height > 1) ? (height >> 1) : 1;
        CreateCubicFilter(height, nheight, (filter & TEX_FILTER_WRAP_V) != 0,
                          (filter & TEX_FILTER_MIRROR_V) != 0, cfY);
        ...
        for (size_t y = 0; y < nheight; ++y)
        {
            ...
            if (toY.u1 != u1)
            {
                if (toY.u1 != u2 && toY.u1 != u3)
                {
                    ...
                    if (! LoadScanlineLinear(row1, width, pSrc + (rowPitch * u1), rowPitch, src->format,
                                             filter))
                        return E_FAIL;
                }
                else if (toY.u1 == u2)
            ...
            if (toY.u2 != u2)
            {
                if (toY.u2 != u3)
                {
                    ...
                    if (! LoadScanlineLinear(row2, width, pSrc + (rowPitch * u2), rowPitch, src->format,
                                             filter))
                        return E_FAIL;
                }
                else
...
//--- 2D Triangle Filter ---
HRESULT Generate2DMipsTriangleFilter(size_t levels, TEX_FILTER_FLAGS filter, const ScratchImage& mipChain,
                                     size_t item) noexcept
{
    using namespace DirectX::Filters;
    ...
    for (size_t level = 1; level < levels; ++level)
    {
        ...
        auto xFromEnd =
          reinterpret_cast<const FilterFrom*>(reinterpret_cast<const uint8_t*>(tfX.get()) + tfX->sizeInBytes);
        auto yFromEnd =
          reinterpret_cast<const FilterFrom*>(reinterpret_cast<const uint8_t*>(tfY.get()) + tfY->sizeInBytes);
        ...
        for (FilterFrom* yFrom = tfY->from; yFrom < yFromEnd;)
        {
            ...
            for (size_t j = 0; j < yFrom->count; ++j)
            {
                ...
                if (! rowAcc->remaining)
                {
                    ...
                    if (! StoreScanlineLinear(pDest + (dest->rowPitch * v), dest->rowPitch, dest->format,
                                              pAccSrc, dest->width, filter))
                        return E_FAIL;
...
//-------------------------------------------------------------------------------------
HRESULT Setup3DMips(_In_reads_(depth) const Image* baseImages, size_t depth, size_t levels,
...
//--- 3D Box Filter ---
HRESULT Generate3DMipsBoxFilter(size_t depth, size_t levels, TEX_FILTER_FLAGS filter,
                                const ScratchImage& mipChain) noexcept
{
    using namespace DirectX::Filters;
            ...
            for (size_t slice = 0; slice < ndepth; ++slice)
            {
                ...
                for (size_t y = 0; y < nheight; ++y)
                {
                    ...
                    for (size_t x = 0; x < nwidth; ++x)
                    {
                        ...
                        AVERAGE8(target[x], urow0[x2], urow1[x2], urow2[x2], urow3[x2], vrow0[x2], vrow1[x2],
                                 vrow2[x2], vrow3[x2])
                    }
...
//--- 3D Linear Filter ---
HRESULT Generate3DMipsLinearFilter(size_t depth, size_t levels, TEX_FILTER_FLAGS filter,
                                   const ScratchImage& mipChain) noexcept
{
    using namespace DirectX::Filters;
                ...
                for (size_t y = 0; y < nheight; ++y)
                {
                    ...
                    if (toY.u0 != u0)
                    {
                        if (toY.u0 != u1)
                        {
                            ...
                            if (! LoadScanlineLinear(urow0, width, srca->pixels + (srca->rowPitch * u0),
                                                     srca->rowPitch, srca->format, filter) ||
                                ! LoadScanlineLinear(vrow0, width, srcb->pixels + (srcb->rowPitch * u0),
                                                     srcb->rowPitch, srcb->format, filter))
                                return E_FAIL;
                        }
                        else
                        {
                    ...
                    if (toY.u1 != u1)
                    {
                        ...
                        if (! LoadScanlineLinear(urow1, width, srca->pixels + (srca->rowPitch * u1),
                                                 srca->rowPitch, srca->format, filter) ||
                            ! LoadScanlineLinear(vrow1, width, srcb->pixels + (srcb->rowPitch * u1),
                                                 srcb->rowPitch, srcb->format, filter))
                            return E_FAIL;
                    }
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                if (toY.u0 != u0)
                {
                    if (toY.u0 != u1)
                    {
                        ...
                        if (! LoadScanlineLinear(urow0, width, pSrc + (rowPitch * u0), rowPitch, src->format,
                                                 filter))
                            return E_FAIL;
                    }
                    else
                ...
                if (toY.u1 != u1)
                {
                    ...
                    if (! LoadScanlineLinear(urow1, width, pSrc + (rowPitch * u1), rowPitch, src->format,
                                             filter))
                        return E_FAIL;
                }
...
//--- 3D Cubic Filter ---
HRESULT Generate3DMipsCubicFilter(size_t depth, size_t levels, TEX_FILTER_FLAGS filter,
                                  const ScratchImage& mipChain) noexcept
{
    using namespace DirectX::Filters;
    ...
    for (size_t level = 1; level < levels; ++level)
    {
        ...
        if (depth > 1)
        {
            // 3D cubic filter
            const size_t ndepth = depth >> 1;
            CreateCubicFilter(depth, ndepth, (filter & TEX_FILTER_WRAP_W) != 0,
                              (filter & TEX_FILTER_MIRROR_W) != 0, cfZ);
                ...
                for (size_t y = 0; y < nheight; ++y)
                {
                    ...
                    if (toY.u0 != u0)
                    {
                        if (toY.u0 != u1 && toY.u0 != u2 && toY.u0 != u3)
                        {
                            ...
                            if (! LoadScanlineLinear(urow[0], width, srca->pixels + (srca->rowPitch * u0),
                                                     srca->rowPitch, srca->format, filter) ||
                                ! LoadScanlineLinear(urow[1], width, srcb->pixels + (srcb->rowPitch * u0),
                                                     srcb->rowPitch, srcb->format, filter) ||
                                ! LoadScanlineLinear(urow[2], width, srcc->pixels + (srcc->rowPitch * u0),
                                                     srcc->rowPitch, srcc->format, filter) ||
                                ! LoadScanlineLinear(urow[3], width, srcd->pixels + (srcd->rowPitch * u0),
                                                     srcd->rowPitch, srcd->format, filter))
                                return E_FAIL;
                        }
                        else if (toY.u0 == u1)
                        {
                    ...
                    if (toY.u1 != u1)
                    {
                        if (toY.u1 != u2 && toY.u1 != u3)
                        {
                            ...
                            if (! LoadScanlineLinear(vrow[0], width, srca->pixels + (srca->rowPitch * u1),
                                                     srca->rowPitch, srca->format, filter) ||
                                ! LoadScanlineLinear(vrow[1], width, srcb->pixels + (srcb->rowPitch * u1),
                                                     srcb->rowPitch, srcb->format, filter) ||
                                ! LoadScanlineLinear(vrow[2], width, srcc->pixels + (srcc->rowPitch * u1),
                                                     srcc->rowPitch, srcc->format, filter) ||
                                ! LoadScanlineLinear(vrow[3], width, srcd->pixels + (srcd->rowPitch * u1),
                                                     srcd->rowPitch, srcd->format, filter))
                                return E_FAIL;
                        }
                        else if (toY.u1 == u2)
                    ...
                    if (toY.u2 != u2)
                    {
                        if (toY.u2 != u3)
                        {
                            ...
                            if (! LoadScanlineLinear(srow[0], width, srca->pixels + (srca->rowPitch * u2),
                                                     srca->rowPitch, srca->format, filter) ||
                                ! LoadScanlineLinear(srow[1], width, srcb->pixels + (srcb->rowPitch * u2),
                                                     srcb->rowPitch, srcb->format, filter) ||
                                ! LoadScanlineLinear(srow[2], width, srcc->pixels + (srcc->rowPitch * u2),
                                                     srcc->rowPitch, srcc->format, filter) ||
                                ! LoadScanlineLinear(srow[3], width, srcd->pixels + (srcd->rowPitch * u2),
                                                     srcd->rowPitch, srcd->format, filter))
                                return E_FAIL;
                        }
                        else
                    ...
                    if (toY.u3 != u3)
                    {
                        ...
                        if (! LoadScanlineLinear(trow[0], width, srca->pixels + (srca->rowPitch * u3),
                                                 srca->rowPitch, srca->format, filter) ||
                            ! LoadScanlineLinear(trow[1], width, srcb->pixels + (srcb->rowPitch * u3),
                                                 srcb->rowPitch, srcb->format, filter) ||
                            ! LoadScanlineLinear(trow[2], width, srcc->pixels + (srcc->rowPitch * u3),
                                                 srcc->rowPitch, srcc->format, filter) ||
                            ! LoadScanlineLinear(trow[3], width, srcd->pixels + (srcd->rowPitch * u3),
                                                 srcd->rowPitch, srcd->format, filter))
                            return E_FAIL;
                    }
                    ...
                    for (size_t x = 0; x < nwidth; ++x)
                    {
                        ...
                        {
                            XMVECTOR C0, C1, C2, C3;
                            CUBIC_INTERPOLATE(C0, toX.x, urow[j][toX.u0], urow[j][toX.u1], urow[j][toX.u2],
                                              urow[j][toX.u3]);
                            CUBIC_INTERPOLATE(C1, toX.x, vrow[j][toX.u0], vrow[j][toX.u1], vrow[j][toX.u2],
                                              vrow[j][toX.u3]);
                            CUBIC_INTERPOLATE(C2, toX.x, srow[j][toX.u0], srow[j][toX.u1], srow[j][toX.u2],
                                              srow[j][toX.u3]);
                            CUBIC_INTERPOLATE(C3, toX.x, trow[j][toX.u0], trow[j][toX.u1], trow[j][toX.u2],
                                              trow[j][toX.u3]);
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                if (toY.u0 != u0)
                {
                    if (toY.u0 != u1 && toY.u0 != u2 && toY.u0 != u3)
                    {
                        ...
                        if (! LoadScanlineLinear(urow[0], width, pSrc + (rowPitch * u0), rowPitch,
                                                 src->format, filter))
                            return E_FAIL;
                    }
                    else if (toY.u0 == u1)
                ...
                if (toY.u1 != u1)
                {
                    if (toY.u1 != u2 && toY.u1 != u3)
                    {
                        ...
                        if (! LoadScanlineLinear(vrow[0], width, pSrc + (rowPitch * u1), rowPitch,
                                                 src->format, filter))
                            return E_FAIL;
                    }
                    else if (toY.u1 == u2)
                ...
                if (toY.u2 != u2)
                {
                    if (toY.u2 != u3)
                    {
                        ...
                        if (! LoadScanlineLinear(srow[0], width, pSrc + (rowPitch * u2), rowPitch,
                                                 src->format, filter))
                            return E_FAIL;
                    }
                    else
                ...
                if (toY.u3 != u3)
                {
                    ...
                    if (! LoadScanlineLinear(trow[0], width, pSrc + (rowPitch * u3), rowPitch, src->format,
                                             filter))
                        return E_FAIL;
                }
                ...
                for (size_t x = 0; x < nwidth; ++x)
                {
                    ...
                    XMVECTOR C0, C1, C2, C3;
                    CUBIC_INTERPOLATE(C0, toX.x, urow[0][toX.u0], urow[0][toX.u1], urow[0][toX.u2],
                                      urow[0][toX.u3]);
                    CUBIC_INTERPOLATE(C1, toX.x, vrow[0][toX.u0], vrow[0][toX.u1], vrow[0][toX.u2],
                                      vrow[0][toX.u3]);
                    CUBIC_INTERPOLATE(C2, toX.x, srow[0][toX.u0], srow[0][toX.u1], srow[0][toX.u2],
                                      srow[0][toX.u3]);
                    CUBIC_INTERPOLATE(C3, toX.x, trow[0][toX.u0], trow[0][toX.u1], trow[0][toX.u2],
                                      trow[0][toX.u3]);
...
//--- 3D Triangle Filter ---
HRESULT Generate3DMipsTriangleFilter(size_t depth, size_t levels, TEX_FILTER_FLAGS filter,
                                     const ScratchImage& mipChain) noexcept
{
    using namespace DirectX::Filters;
    ...
    for (size_t level = 1; level < levels; ++level)
    {
        ...
        auto yFromEnd =
          reinterpret_cast<const FilterFrom*>(reinterpret_cast<const uint8_t*>(tfY.get()) + tfY->sizeInBytes);
        auto zFromEnd =
          reinterpret_cast<const FilterFrom*>(reinterpret_cast<const uint8_t*>(tfZ.get()) + tfZ->sizeInBytes);
        ...
        for (FilterFrom* zFrom = tfZ->from; zFrom < zFromEnd; ++z)
        {
            ...
            for (FilterFrom* yFrom = tfY->from; yFrom < yFromEnd;)
            {
                ...
                for (FilterFrom* xFrom = tfX->from; xFrom < xFromEnd; ++x)
                {
                    ...
                    xFrom =
                      reinterpret_cast<FilterFrom*>(reinterpret_cast<uint8_t*>(xFrom) + xFrom->sizeInBytes);
                }
            ...
            for (size_t j = 0; j < zFrom->count; ++j)
            {
                ...
                if (! sliceAcc->remaining)
                {
                    ...
                    for (size_t h = 0; h < nheight; ++h)
                    {
                        ...
                        // This performs any required clamping
                        if (! StoreScanlineLinear(pDest, dest->rowPitch, dest->format, pAccSrc, dest->width,
                                                  filter))
                            return E_FAIL;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::GenerateMipMaps(const Image& baseImage, TEX_FILTER_FLAGS filter,
                                                        size_t levels, ScratchImage& mipChain,
                                                        bool allow1D) noexcept
{
    ...
    if (IsCompressed(baseImage.format) || IsTypeless(baseImage.format) || IsPlanar(baseImage.format) ||
        IsPalettized(baseImage.format))
    {
    ...
    if (usewic && ! wicpf)
    {
        ...
        const uint64_t expandedSize2 =
          uint64_t(baseImage.width) * uint64_t(baseImage.height) * sizeof(float) * 4;
    ...
    if (usewic)
    {
        ...
        case TEX_FILTER_CUBIC:
        {
            ...
            else
            {
                ...
                hr = (baseImage.height > 1 || ! allow1D)
                  ? tMipChain.Initialize2D(DXGI_FORMAT_R32G32B32A32_FLOAT, baseImage.width, baseImage.height,
                                           1, levels)
                  : tMipChain.Initialize1D(DXGI_FORMAT_R32G32B32A32_FLOAT, baseImage.width, 1, levels);
                if (FAILED(hr))
                    return hr;

                hr = GenerateMipMapsUsingWIC(*timg, filter, levels, GUID_WICPixelFormat128bppRGBAFloat,
                                             tMipChain, 0);
                if (FAILED(hr))
                    return hr;
                ...
                return ConvertFromR32G32B32A32(tMipChain.GetImages(), tMipChain.GetImageCount(),
                                               tMipChain.GetMetadata(), baseImage.format, mipChain);
            }
        }
...
#endif // WIN32
    {
        ...
        if (! filter_select)
        {
            ...
            filter_select =
              (ispow2(baseImage.width) && ispow2(baseImage.height)) ? TEX_FILTER_BOX : TEX_FILTER_LINEAR;
        }
...
_Use_decl_annotations_ HRESULT DirectX::GenerateMipMaps(const Image* srcImages, size_t nimages,
                                                        const TexMetadata& metadata, TEX_FILTER_FLAGS filter,
                                                        size_t levels, ScratchImage& mipChain)
{
    if (! srcImages || ! nimages || ! IsValid(metadata.format))
        return E_INVALIDARG;

    if (metadata.IsVolumemap() || IsCompressed(metadata.format) || IsTypeless(metadata.format) ||
        IsPlanar(metadata.format) || IsPalettized(metadata.format))
        return HRESULT_E_NOT_SUPPORTED;
    ...
    {
        // Check to see if the source and/or result size is too big for WIC
        const uint64_t expandedSize = uint64_t(std::max<size_t>(1, metadata.width >> 1)) *
          uint64_t(std::max<size_t>(1, metadata.height >> 1)) * sizeof(float) * 4;
        const uint64_t expandedSize2 =
          uint64_t(metadata.width) * uint64_t(metadata.height) * sizeof(float) * 4;
        if (expandedSize > UINT32_MAX || expandedSize2 > UINT32_MAX)
        {
            if (filter & TEX_FILTER_FORCE_WIC)
                return HRESULT_E_ARITHMETIC_OVERFLOW;
    ...
    if (usewic)
    {
        ...
        case TEX_FILTER_CUBIC:
        {
            ...
            else
            {
                ...
                return ConvertFromR32G32B32A32(tMipChain.GetImages(), tMipChain.GetImageCount(),
                                               tMipChain.GetMetadata(), metadata.format, mipChain);
            }
        }
...
#endif // WIN32
    {
        ...
        if (! filter_select)
        {
            // Default filter choice
            filter_select =
              (ispow2(metadata.width) && ispow2(metadata.height)) ? TEX_FILTER_BOX : TEX_FILTER_LINEAR;
        }
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::GenerateMipMaps3D(const Image* baseImages, size_t depth,
                                                          ...
                                                          ScratchImage& mipChain) noexcept
{
    ...
    for (size_t slice = 0; slice < depth; ++slice)
    {
        ...
        if (baseImages[slice].format != format || baseImages[slice].width != width ||
            baseImages[slice].height != height)
        {
            // All base images must be the same format, width, and height
            return E_FAIL;
        }
    ...
    if (! filter_select)
    {
        // Default filter choice
        filter_select =
          (ispow2(width) && ispow2(height) && ispow2(depth)) ? TEX_FILTER_BOX : TEX_FILTER_TRIANGLE;
    }
...
_Use_decl_annotations_ HRESULT DirectX::GenerateMipMaps3D(const Image* srcImages, size_t nimages,
                                                          ...
                                                          ScratchImage& mipChain)
{
    ...
    {
        // Default filter choice
        filter_select = (ispow2(metadata.width) && ispow2(metadata.height) && ispow2(metadata.depth))
          ? TEX_FILTER_BOX
          : TEX_FILTER_TRIANGLE;
    }
...
_Use_decl_annotations_ HRESULT DirectX::ScaleMipMapsAlphaForCoverage(const Image* srcImages, size_t nimages,
                                                                     const TexMetadata& metadata, size_t item,
                                                                     float alphaReference,
                                                                     ScratchImage& mipChain) noexcept
{
    if (! srcImages || ! nimages || ! IsValid(metadata.format) || nimages > metadata.mipLevels ||
        ! mipChain.GetImages())
        return E_INVALIDARG;
