...
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE1D) ==
                static_cast<int>(D3D11_RESOURCE_DIMENSION_TEXTURE1D),
              "header enum mismatch");
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE2D) ==
                static_cast<int>(D3D11_RESOURCE_DIMENSION_TEXTURE2D),
              "header enum mismatch");
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE3D) ==
                static_cast<int>(D3D11_RESOURCE_DIMENSION_TEXTURE3D),
              "header enum mismatch");

namespace
{
HRESULT Capture(_In_ ID3D11DeviceContext* pContext, _In_ ID3D11Resource* pSource, const TexMetadata& metadata,
                const ScratchImage& result) noexcept
{
    ...
    if (metadata.IsVolumemap())
    {
        ...
        {
            const UINT dindex =
              D3D11CalcSubresource(static_cast<UINT>(level), 0, static_cast<UINT>(metadata.mipLevels));
    ...
    else
    {
        ...
        for (size_t item = 0; item < metadata.arraySize; ++item)
        {
            ...
            for (size_t level = 0; level < metadata.mipLevels; ++level)
            {
                const UINT dindex = D3D11CalcSubresource(static_cast<UINT>(level), static_cast<UINT>(item),
                                                        ...
                                                        const TexMetadata& metadata) noexcept
{
    ...
    case TEX_DIMENSION_TEXTURE2D:
        if (metadata.IsCubemap())
        {
            ...
            if ((arraySize > D3D11_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION) ||
                (iWidth > D3D11_REQ_TEXTURECUBE_DIMENSION) || (iHeight > D3D11_REQ_TEXTURECUBE_DIMENSION))
                return false;

            if (fl < D3D_FEATURE_LEVEL_11_0)
            {
                if ((arraySize > D3D10_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION) ||
                    (iWidth > D3D10_REQ_TEXTURECUBE_DIMENSION) || (iHeight > D3D10_REQ_TEXTURECUBE_DIMENSION))
                    return false;
    ...
    case TEX_DIMENSION_TEXTURE3D:
        ...
        if ((arraySize > 1) || (iWidth > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION) ||
...
{
    return CreateTextureEx(pDevice, srcImages, nimages, metadata, D3D11_USAGE_DEFAULT,
                           D3D11_BIND_SHADER_RESOURCE, 0, 0, CREATETEX_DEFAULT, ppResource);
}
                                                        ...
                                                        ID3D11Resource** ppResource) noexcept
{
    ...
    *ppResource = nullptr;
    ...
    std::unique_ptr<D3D11_SUBRESOURCE_DATA[]> initData(
      new (std::nothrow) D3D11_SUBRESOURCE_DATA[metadata.mipLevels * metadata.arraySize]);
    if (! initData)
    ...
    if (metadata.IsVolumemap())
    {
        ...
        for (size_t level = 0; level < metadata.mipLevels; ++level)
        {
            ...
            for (size_t slice = 1; slice < depth; ++slice)
            {
                ...
                if (timg.pixels != pSlice || timg.format != metadata.format ||
                    timg.rowPitch != img.rowPitch || timg.slicePitch != img.slicePitch)
                    return E_FAIL;
...
{
    return CreateShaderResourceViewEx(pDevice, srcImages, nimages, metadata, D3D11_USAGE_DEFAULT,
                                      D3D11_BIND_SHADER_RESOURCE, 0, 0, CREATETEX_DEFAULT, ppSRV);
}

_Use_decl_annotations_ HRESULT DirectX::CreateShaderResourceViewEx(
  ID3D11Device* pDevice, const Image* srcImages, size_t nimages, const TexMetadata& metadata,
  D3D11_USAGE usage, unsigned int bindFlags, unsigned int cpuAccessFlags, unsigned int miscFlags,
  CREATETEX_FLAGS flags, ID3D11ShaderResourceView** ppSRV) noexcept
{
    if (! ppSRV)
        return E_INVALIDARG;
    ...
    *ppSRV = nullptr;
    ...
    HRESULT hr = CreateTextureEx(pDevice, srcImages, nimages, metadata, usage, bindFlags, cpuAccessFlags,
                                 miscFlags, flags, resource.GetAddressOf());
    if (FAILED(hr))
        return hr;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::CaptureTexture(ID3D11Device* pDevice, ID3D11DeviceContext* pContext,
                                                       ID3D11Resource* pSource, ScratchImage& result) noexcept
{
