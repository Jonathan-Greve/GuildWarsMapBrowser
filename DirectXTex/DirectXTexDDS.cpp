...
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE1D) == static_cast<int>(DDS_DIMENSION_TEXTURE1D),
              "header enum mismatch");
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE2D) == static_cast<int>(DDS_DIMENSION_TEXTURE2D),
              "header enum mismatch");
static_assert(static_cast<int>(TEX_DIMENSION_TEXTURE3D) == static_cast<int>(DDS_DIMENSION_TEXTURE3D),
              "header enum mismatch");
...
const LegacyDDS g_LegacyDDSMap[] = {
  ...
  {DXGI_FORMAT_BC6H_UF16,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, MAKEFOURCC('B', 'C', '6', 'H'), 0, 0, 0, 0, 0}},
  {DXGI_FORMAT_BC7_UNORM,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, MAKEFOURCC('B', 'C', '7', 'L'), 0, 0, 0, 0, 0}},
  {DXGI_FORMAT_BC7_UNORM,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, MAKEFOURCC('B', 'C', '7', '\0'), 0, 0, 0, 0, 0}},
  ...
  {DXGI_FORMAT_R16G16B16A16_UNORM,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 36, 0, 0, 0, 0, 0}}, // D3DFMT_A16B16G16R16
  {DXGI_FORMAT_R16G16B16A16_SNORM,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 110, 0, 0, 0, 0, 0}}, // D3DFMT_Q16W16V16U16
  {DXGI_FORMAT_R16_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 111, 0, 0, 0, 0, 0}}, // D3DFMT_R16F
  {DXGI_FORMAT_R16G16_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 112, 0, 0, 0, 0, 0}}, // D3DFMT_G16R16F
  {DXGI_FORMAT_R16G16B16A16_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 113, 0, 0, 0, 0, 0}}, // D3DFMT_A16B16G16R16F
  {DXGI_FORMAT_R32_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 114, 0, 0, 0, 0, 0}}, // D3DFMT_R32F
  {DXGI_FORMAT_R32G32_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 115, 0, 0, 0, 0, 0}}, // D3DFMT_G32R32F
  {DXGI_FORMAT_R32G32B32A32_FLOAT,
   CONV_FLAGS_NONE,
   {sizeof(DDS_PIXELFORMAT), DDS_FOURCC, 116, 0, 0, 0, 0, 0}}, // D3DFMT_A32B32G32R32F
  ...
  {DXGI_FORMAT_R8G8B8A8_UNORM,
   ...
   {sizeof(DDS_PIXELFORMAT), DDS_PAL8A, 0, 16, 0, 0, 0, 0}}, // D3DFMT_A8P8
  {DXGI_FORMAT_R8G8B8A8_UNORM,
...
DXGI_FORMAT GetDXGIFormat(const DDS_HEADER& hdr, const DDS_PIXELFORMAT& ddpf, DDS_FLAGS flags,
                          _Inout_ uint32_t& convFlags) noexcept
{
    ...
    for (index = 0; index < MAP_SIZE; ++index)
    {
        ...
        else if (ddpfFlags == entry->ddpf.flags)
        {
            ...
            {
                if (ddpf.RGBBitCount == entry->ddpf.RGBBitCount && ddpf.ABitMask == entry->ddpf.ABitMask)
                    break;
            }
            else if (entry->ddpf.flags & DDS_LUMINANCE)
            {
                if (entry->ddpf.flags & DDS_ALPHAPIXELS)
                {
                    // LUMINANCEA
                    if (ddpf.RGBBitCount == entry->ddpf.RGBBitCount &&
                        ddpf.RBitMask == entry->ddpf.RBitMask && ddpf.ABitMask == entry->ddpf.ABitMask)
                        break;
                }
                else
                {
                    // LUMINANCE
                    if (ddpf.RGBBitCount == entry->ddpf.RGBBitCount && ddpf.RBitMask == entry->ddpf.RBitMask)
                        break;
                }
            }
            ...
            {
                if (ddpf.RGBBitCount == entry->ddpf.RGBBitCount && ddpf.RBitMask == entry->ddpf.RBitMask &&
                    ddpf.GBitMask == entry->ddpf.GBitMask && ddpf.BBitMask == entry->ddpf.BBitMask &&
                        ...
                        _Out_ TexMetadata& metadata, _Inout_ uint32_t& convFlags) noexcept
{
    ...
    auto pHeader =
      reinterpret_cast<const DDS_HEADER*>(static_cast<const uint8_t*>(pSource) + sizeof(uint32_t));
    ...
    if ((pHeader->ddspf.flags & DDS_FOURCC) && (MAKEFOURCC('D', 'X', '1', '0') == pHeader->ddspf.fourCC))
    {
        ...
        auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10*>(static_cast<const uint8_t*>(pSource) +
                                                        ...
                                                        size_t& required) noexcept
{
    ...
    if (! (flags & DDS_FLAGS_FORCE_DX10_EXT))
    {
        ...
        case DXGI_FORMAT_R16G16_UNORM:
            memcpy(&ddpf, &DDSPF_G16R16, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R8G8_UNORM:
            memcpy(&ddpf, &DDSPF_A8L8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R16_UNORM:
            memcpy(&ddpf, &DDSPF_L16, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R8_UNORM:
            memcpy(&ddpf, &DDSPF_L8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_A8_UNORM:
            memcpy(&ddpf, &DDSPF_A8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R8G8_B8G8_UNORM:
            memcpy(&ddpf, &DDSPF_R8G8_B8G8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_G8R8_G8B8_UNORM:
            memcpy(&ddpf, &DDSPF_G8R8_G8B8, sizeof(DDS_PIXELFORMAT));
            break;
        ...
        case DXGI_FORMAT_BC4_SNORM:
            memcpy(&ddpf, &DDSPF_BC4_SNORM, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_BC5_SNORM:
            memcpy(&ddpf, &DDSPF_BC5_SNORM, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_B5G6R5_UNORM:
            memcpy(&ddpf, &DDSPF_R5G6B5, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_B5G5R5A1_UNORM:
            memcpy(&ddpf, &DDSPF_A1R5G5B5, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R8G8_SNORM:
            memcpy(&ddpf, &DDSPF_V8U8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R8G8B8A8_SNORM:
            memcpy(&ddpf, &DDSPF_Q8W8V8U8, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_R16G16_SNORM:
            memcpy(&ddpf, &DDSPF_V16U16, sizeof(DDS_PIXELFORMAT));
            break;
        case DXGI_FORMAT_B8G8R8A8_UNORM:
    ...
    *static_cast<uint32_t*>(pDestination) = DDS_MAGIC;
    ...
    if (ddpf.size == 0)
    {
        ...
        auto ext =
          reinterpret_cast<DDS_HEADER_DXT10*>(reinterpret_cast<uint8_t*>(header) + sizeof(DDS_HEADER));
        assert(ext);
...
_Success_(return) bool LegacyExpandScanline(_Out_writes_bytes_(outSize) void* pDestination, size_t outSize,
                                            _In_ DXGI_FORMAT outFormat,
                                            _In_reads_bytes_(inSize) const void* pSource, size_t inSize,
                                            ...
                                            _In_ uint32_t tflags) noexcept
{
    ...
    case TEXP_LEGACY_R8G8B8:
        ...
        if (inSize >= 3 && outSize >= 4)
        {
            ...
            for (size_t ocount = 0, icount = 0; ((icount < (inSize - 2)) && (ocount < (outSize - 3)));
                 icount += 3, ocount += 4)
            {
    ...
    case TEXP_LEGACY_R3G3B2:
        ...
        case DXGI_FORMAT_R8G8B8A8_UNORM:
            ...
            if (inSize >= 1 && outSize >= 4)
            {
                ...
                for (size_t ocount = 0, icount = 0; ((icount < inSize) && (ocount < (outSize - 3)));
                     ++icount, ocount += 4)
                {
                    ...
                    uint32_t t3 = uint32_t(((t & 0x03) << 22) | ((t & 0x03) << 20) | ((t & 0x03) << 18) |
                                           ((t & 0x03) << 16));
        ...
        case DXGI_FORMAT_B5G6R5_UNORM:
            ...
            if (inSize >= 1 && outSize >= 2)
            {
                ...
                for (size_t ocount = 0, icount = 0; ((icount < inSize) && (ocount < (outSize - 1)));
                     ++icount, ocount += 2)
                {
    ...
    case TEXP_LEGACY_A8R3G3B2:
        ...
        if (inSize >= 2 && outSize >= 4)
        {
                 ...
                 icount += 2, ocount += 4)
            {
                ...
                uint32_t t3 = uint32_t(((t & 0x0003) << 22) | ((t & 0x0003) << 20) | ((t & 0x0003) << 18) |
                                       ((t & 0x0003) << 16));
    ...
    case TEXP_LEGACY_B4G4R4A4:
        ...
        if (inSize >= 2 && outSize >= 4)
        {
                 ...
                 icount += 2, ocount += 4)
            {
                ...
                uint32_t ta =
                  uint32_t((tflags & TEXP_SCANLINE_SETALPHA) ? 0xff000000
    ...
    case TEXP_LEGACY_L16:
        ...
        if (inSize >= 2 && outSize >= 8)
        {
            ...
            for (size_t ocount = 0, icount = 0; ((icount < (inSize - 1)) && (ocount < (outSize - 7)));
                 icount += 2, ocount += 8)
            {
                const uint16_t t = *(sPtr++);
...
//-------------------------------------------------------------------------------------
HRESULT CopyImage(_In_reads_bytes_(size) const void* pPixels, _In_ size_t size,
                  _In_ const TexMetadata& metadata, _In_ CP_FLAGS cpFlags, _In_ uint32_t convFlags,
                  _In_reads_opt_(256) const uint32_t* pal8, _In_ const ScratchImage& image) noexcept
{
    assert(pPixels);
    ...
    if (convFlags & CONV_FLAGS_EXPAND)
    {
        if (convFlags & CONV_FLAGS_888)
            cpFlags |= CP_FLAGS_24BPP;
                 ...
                 (CONV_FLAGS_565 | CONV_FLAGS_5551 | CONV_FLAGS_4444 | CONV_FLAGS_8332 | CONV_FLAGS_A8P8 |
                  CONV_FLAGS_L16 | CONV_FLAGS_A8L8))
    ...
    if (! SetupImageArray(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(pPixels)), pixelSize,
                          metadata, cpFlags, timages.get(), nimages))
    {
        return E_FAIL;
    }
                    ...
                    for (size_t h = 0; h < images[index].height; ++h)
                    {
                        if (convFlags & CONV_FLAGS_EXPAND)
                        {
                            ...
                            else
                            {
                                ...
                                if (! LegacyExpandScanline(pDest, dpitch, metadata.format, pSrc, spitch,
                                                           lformat, pal8, tflags))
                                    return E_FAIL;
                            }
                        }
            ...
            for (size_t slice = 0; slice < d; ++slice, ++index)
            {
                ...
                if (IsCompressed(metadata.format))
                {
                    ...
                    if (cpFlags & CP_FLAGS_BAD_DXTN_TAILS)
                    {
                        if (images[index].width < 4 || images[index].height < 4)
                        {
                            csize = std::min<size_t>(images[index].slicePitch,
                                                     timages[lastgood + slice].slicePitch);
...
_Use_decl_annotations_ HRESULT DirectX::GetMetadataFromDDSMemory(const void* pSource, size_t size,
                                                                 DDS_FLAGS flags,
...
_Use_decl_annotations_ HRESULT DirectX::GetMetadataFromDDSFile(const wchar_t* szFile, DDS_FLAGS flags,
                                                               TexMetadata& metadata) noexcept
{
    if (! szFile)
        return E_INVALIDARG;
    ...
    ScopedHandle hFile(
      safe_handle(CreateFile2(szFile, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr)));
#else
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::LoadFromDDSMemory(const void* pSource, size_t size, DDS_FLAGS flags,
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::LoadFromDDSFile(const wchar_t* szFile, DDS_FLAGS flags,
                                                        TexMetadata* metadata, ScratchImage& image) noexcept
{
    if (! szFile)
        return E_INVALIDARG;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveToDDSMemory(const Image* images, size_t nimages,
                                                        ...
                                                        Blob& blob) noexcept
{
    ...
    for (size_t i = 0; i < nimages; ++i)
    {
        ...
        hr = ComputePitch(metadata.format, images[i].width, images[i].height, ddsRowPitch, ddsSlicePitch,
                          CP_FLAGS_NONE);
        if (FAILED(hr))
            return hr;
    ...
    case DDS_DIMENSION_TEXTURE2D:
    {
        ...
        for (size_t item = 0; item < metadata.arraySize; ++item)
        {
            for (size_t level = 0; level < metadata.mipLevels; ++level)
            {
                ...
                {
                    size_t ddsRowPitch, ddsSlicePitch;
                    hr = ComputePitch(metadata.format, images[index].width, images[index].height, ddsRowPitch,
                                      ddsSlicePitch, CP_FLAGS_NONE);
                    if (FAILED(hr))
                    {
                        blob.Release();
                        return hr;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveToDDSFile(const Image* images, size_t nimages,
                                                      const TexMetadata& metadata, DDS_FLAGS flags,
...
#else
    ScopedHandle hFile(safe_handle(CreateFileW(szFile, GENERIC_WRITE | DELETE, 0, nullptr, CREATE_ALWAYS,
    ...
    case DDS_DIMENSION_TEXTURE2D:
    {
        ...
        for (size_t item = 0; item < metadata.arraySize; ++item)
        {
            for (size_t level = 0; level < metadata.mipLevels; ++level, ++index)
            {
                ...
                {
#ifdef _WIN32
                    if (! WriteFile(hFile.get(), images[index].pixels, static_cast<DWORD>(ddsSlicePitch),
                ...
                else
                {
                    ...
                    {
#ifdef _WIN32
                        if (! WriteFile(hFile.get(), sPtr, static_cast<DWORD>(ddsRowPitch), &bytesWritten,
                                        nullptr))
                        {
                            return HRESULT_FROM_WIN32(GetLastError());
                        }
