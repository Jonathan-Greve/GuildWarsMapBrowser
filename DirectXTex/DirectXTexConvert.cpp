...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ void DirectX::Internal::CopyScanline(void* pDestination, size_t outSize,
                                                            const void* pSource, size_t inSize,
                                                            DXGI_FORMAT format, uint32_t tflags) noexcept
{
        ...
        case DXGI_FORMAT_AYUV:
            if (inSize >= 4 && outSize >= 4)
            {
                const uint32_t alpha =
                  (format == DXGI_FORMAT_R8G8B8A8_SNORM || format == DXGI_FORMAT_R8G8B8A8_SINT) ? 0x7f000000
                                                                                                : 0xff000000;
...
// (can be used in place as well) otherwise copies the image row unmodified.
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ void DirectX::Internal::SwizzleScanline(void* pDestination, size_t outSize,
                                                               const void* pSource, size_t inSize,
                                                               DXGI_FORMAT format, uint32_t tflags) noexcept
{
    assert(pDestination && outSize > 0);
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ bool DirectX::Internal::ExpandScanline(void* pDestination, size_t outSize,
                                                              DXGI_FORMAT outFormat, const void* pSource,
                                                              size_t inSize, DXGI_FORMAT inFormat,
                                                              uint32_t tflags) noexcept
{
    ...
    case DXGI_FORMAT_B5G6R5_UNORM:
        ...
        if (inSize >= 2 && outSize >= 4)
        {
            ...
            for (size_t ocount = 0, icount = 0; ((icount < (inSize - 1)) && (ocount < (outSize - 3)));
                 icount += 2, ocount += 4)
            {
    ...
    case DXGI_FORMAT_B5G5R5A1_UNORM:
        ...
        if (inSize >= 2 && outSize >= 4)
        {
                 ...
                 icount += 2, ocount += 4)
            {
                ...
                uint32_t t3 = uint32_t(((t & 0x001f) << 19) | ((t & 0x001c) << 14));
                uint32_t ta =
                  (tflags & TEXP_SCANLINE_SETALPHA) ? 0xff000000 : ((t & 0x8000) ? 0xff000000 : 0);
        ...
        if (inSize >= 2 && outSize >= 4)
        {
                 ...
                 icount += 2, ocount += 4)
            {
                ...
                uint32_t ta = (tflags & TEXP_SCANLINE_SETALPHA)
                  ? 0xff000000
                  : uint32_t(((t & 0xf000) << 16) | ((t & 0xf000) << 12));
...
_Use_decl_annotations_ bool DirectX::Internal::LoadScanline(XMVECTOR* pDestination, size_t count,
                                                            const void* pSource, size_t size,
                                                            DXGI_FORMAT format) noexcept
{
    assert(pDestination && count > 0 && ((reinterpret_cast<uintptr_t>(pDestination) & 0xF) == 0));
    assert(pSource && size > 0);
    assert(IsValid(format) && ! IsTypeless(format, false) && ! IsCompressed(format) && ! IsPlanar(format) &&
           ! IsPalettized(format));
        ...
        if (size >= psize)
        {
            ...
            for (size_t icount = 0; icount < (size - psize + 1); icount += psize)
            {
                ...
                if (dPtr >= ePtr)
                    break;
                *(dPtr++) =
                  XMVectorSet(0.f /* typeless component assumed zero */, static_cast<float>(*pg8), 0.f, 1.f);
                sPtr += 2;
            }
            return true;
        }
        ...
        if (size >= sizeof(XMUBYTEN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUBYTEN4) + 1); icount += sizeof(XMUBYTEN4))
            {
                ...
                if (dPtr >= ePtr)
                    break;
                *(dPtr++) =
                  XMVectorSet(float(std::min<int>(std::max<int>(r, 0), 255)) / 255.f,
                              float(std::min<int>(std::max<int>(g, 0), 255)) / 255.f,
                              float(std::min<int>(std::max<int>(b, 0), 255)) / 255.f, float(a) / 255.f);
            }
            return true;
        }
        return false;
        ...
        if (size >= sizeof(XMUDECN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUDECN4) + 1); icount += sizeof(XMUDECN4))
            {
                ...
                if (dPtr >= ePtr)
                    break;
                *(dPtr++) =
                  XMVectorSet(float(std::min<int>(std::max<int>(r, 0), 1023)) / 1023.f,
                              float(std::min<int>(std::max<int>(g, 0), 1023)) / 1023.f,
                              float(std::min<int>(std::max<int>(b, 0), 1023)) / 1023.f, float(a) / 3.f);
            }
            return true;
        }
        return false;
        ...
        if (size >= sizeof(XMUBYTEN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUBYTEN4) + 1); icount += sizeof(XMUBYTEN4))
            {
                ...
                *(dPtr++) = XMVectorSet(float(std::min<int>(std::max<int>(r, 0), 255)) / 255.f,
                                        float(std::min<int>(std::max<int>(g, 0), 255)) / 255.f,
                                        float(std::min<int>(std::max<int>(b, 0), 255)) / 255.f, 1.f);
        ...
        if (size >= sizeof(XMUSHORTN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUSHORTN4) + 1); icount += sizeof(XMUSHORTN4))
            {
                ...
                *(dPtr++) = XMVectorSet(float(std::min<int>(std::max<int>(r, 0), 1023)) / 1023.f,
                                        float(std::min<int>(std::max<int>(g, 0), 1023)) / 1023.f,
                                        float(std::min<int>(std::max<int>(b, 0), 1023)) / 1023.f, 1.f);
    ...
    case DXGI_FORMAT_Y216:
        if (size >= sizeof(XMUSHORTN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUSHORTN4) + 1); icount += sizeof(XMUSHORTN4))
            {
                ...
                *(dPtr++) = XMVectorSet(float(std::min<int>(std::max<int>(r, 0), 65535)) / 65535.f,
                                        float(std::min<int>(std::max<int>(g, 0), 65535)) / 65535.f,
                                        float(std::min<int>(std::max<int>(b, 0), 65535)) / 65535.f, 1.f);
    ...
    case XBOX_DXGI_FORMAT_R10G10B10_7E3_A2_FLOAT:
        ...
        if (size >= sizeof(XMUDECN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUDECN4) + 1); icount += sizeof(XMUDECN4))
            {
                ...
                XMVECTORF32 vResult = {{{FloatFrom7e3(sPtr->x), FloatFrom7e3(sPtr->y), FloatFrom7e3(sPtr->z),
    ...
    case XBOX_DXGI_FORMAT_R10G10B10_6E4_A2_FLOAT:
        ...
        if (size >= sizeof(XMUDECN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUDECN4) + 1); icount += sizeof(XMUDECN4))
            {
                ...
                XMVECTORF32 vResult = {{{FloatFrom6e4(sPtr->x), FloatFrom6e4(sPtr->y), FloatFrom6e4(sPtr->z),
                                         static_cast<float>(sPtr->v >> 30) / 3.0f}}};
...
_Use_decl_annotations_ bool DirectX::Internal::StoreScanline(void* pDestination, size_t size,
                                                             DXGI_FORMAT format, const XMVECTOR* pSource,
                                                             size_t count, float threshold) noexcept
{
    assert(pDestination != nullptr);
    assert(IsValid(format) && ! IsTypeless(format) && ! IsCompressed(format) && ! IsPlanar(format) &&
           ! IsPalettized(format));
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Internal::ConvertToR32G32B32A32(const Image& srcImage,
                                                                        ScratchImage& image) noexcept
{
    ...
    {
        if (! LoadScanline(reinterpret_cast<XMVECTOR*>(pDest), srcImage.width, pSrc, srcImage.rowPitch,
                           srcImage.format))
        {
            image.Release();
            return E_FAIL;
                                                                          ...
                                                                          const Image& destImage) noexcept
{
    ...
    for (size_t h = 0; h < srcImage.height; ++h)
    {
        if (! StoreScanline(pDest, destImage.rowPitch, destImage.format,
                            reinterpret_cast<const XMVECTOR*>(pSrc), srcImage.width))
            return E_FAIL;
                                                                          ...
                                                                          ScratchImage& result) noexcept
{
    ...
    for (size_t index = 0; index < nimages; ++index)
    {
        ...
        {
            if (! StoreScanline(pDest, dst.rowPitch, format, reinterpret_cast<const XMVECTOR*>(pSrc),
                                src.width))
            {
                result.Release();
                return E_FAIL;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Internal::ConvertToR16G16B16A16(const Image& srcImage,
                                                                        ScratchImage& image) noexcept
{
    if (! srcImage.pixels)
        return E_POINTER;
    ...
    for (size_t h = 0; h < srcImage.height; ++h)
    {
        ...
        XMConvertFloatToHalfStream(reinterpret_cast<HALF*>(pDest), sizeof(HALF),
...
_Use_decl_annotations_ HRESULT DirectX::Internal::ConvertFromR16G16B16A16(const Image& srcImage,
                                                                          const Image& destImage) noexcept
{
    ...
    for (size_t h = 0; h < srcImage.height; ++h)
    {
        XMConvertHalfToFloatStream(reinterpret_cast<float*>(scanline.get()), sizeof(float),
                                   reinterpret_cast<const HALF*>(pSrc), sizeof(HALF), srcImage.width * 4);
...
//                             where a = 0.055
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ bool DirectX::Internal::StoreScanlineLinear(void* pDestination, size_t size,
                                                                   DXGI_FORMAT format, XMVECTOR* pSource,
                                                                   size_t count, TEX_FILTER_FLAGS flags,
...
//                         where a = 0.055
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ bool DirectX::Internal::LoadScanlineLinear(XMVECTOR* pDestination, size_t count,
                                                                  const void* pSource, size_t size,
                                                                  DXGI_FORMAT format,
...
const ConvertData g_ConvertTable[] = {
  ...
  {DXGI_FORMAT_R9G9B9E5_SHAREDEXP, 9,
   CONVF_FLOAT | CONVF_SHAREDEXP | CONVF_POS_ONLY | CONVF_R | CONVF_G | CONVF_B},
  ...
  {DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM, 10,
   CONVF_UNORM | CONVF_XR | CONVF_R | CONVF_G | CONVF_B | CONVF_A},
  ...
  {XBOX_DXGI_FORMAT_R10G10B10_7E3_A2_FLOAT, 10,
   CONVF_FLOAT | CONVF_POS_ONLY | CONVF_R | CONVF_G | CONVF_B | CONVF_A},
  {XBOX_DXGI_FORMAT_R10G10B10_6E4_A2_FLOAT, 10,
   CONVF_FLOAT | CONVF_POS_ONLY | CONVF_R | CONVF_G | CONVF_B | CONVF_A},
...
_Use_decl_annotations_ void DirectX::Internal::ConvertScanline(XMVECTOR* pBuffer, size_t count,
                                                               DXGI_FORMAT outFormat, DXGI_FORMAT inFormat,
                                                               TEX_FILTER_FLAGS flags) noexcept
{
    assert(pBuffer && count > 0 && ((reinterpret_cast<uintptr_t>(pBuffer) & 0xF) == 0));
    assert(IsValid(outFormat) && ! IsTypeless(outFormat) && ! IsPlanar(outFormat) &&
    ...
    if (diffFlags != 0)
    {
        if (diffFlags & CONVF_DEPTH)
        {
            ...
            else
            {
                ...
                default:
                    if ((in->flags & CONVF_UNORM) &&
                        ((in->flags & CONVF_RGB_MASK) == (CONVF_R | CONVF_G | CONVF_B)))
                    {
                        XMVECTOR* ptr = pBuffer;
                        for (size_t i = 0; i < count; ++i)
                        {
                ...
                if (out->flags & CONVF_STENCIL)
                {
                    ...
                    static const XMVECTORU32 select0100 = {
        ...
        else if ((in->flags & CONVF_RGB_MASK) == (CONVF_R | CONVF_G | CONVF_B))
        {
            ...
            else if ((out->flags & CONVF_RGB_MASK) == (CONVF_R | CONVF_G))
            {
                ...
                switch (static_cast<int>(
                  flags & (TEX_FILTER_RGB_COPY_RED | TEX_FILTER_RGB_COPY_GREEN | TEX_FILTER_RGB_COPY_BLUE)))
                {
                ...
                case (static_cast<int>(TEX_FILTER_RGB_COPY_GREEN) |
                      static_cast<int>(TEX_FILTER_RGB_COPY_BLUE)):
                {
                    XMVECTOR* ptr = pBuffer;
                    for (size_t i = 0; i < count; ++i)
                    {
                ...
                case (static_cast<int>(TEX_FILTER_RGB_COPY_RED) |
                      static_cast<int>(TEX_FILTER_RGB_COPY_GREEN)):
                default:
                    // Leave data unchanged and the store will handle this...
                    break;
                }
    ...
    {                                                                                                        \
        ...
        {                                                                                                    \
            ...
            {                                                                                                \
                ...
                pDiffusionErrors[index - delta] =                                                            \
                  XMVectorMultiplyAdd(g_ErrorWeight3, vError, pDiffusionErrors[index - delta]);              \
                pDiffusionErrors[index + 1] =                                                                \
                  XMVectorMultiplyAdd(g_ErrorWeight5, vError, pDiffusionErrors[index + 1]);                  \
                pDiffusionErrors[index + 2 + delta] =                                                        \
                  XMVectorMultiplyAdd(g_ErrorWeight1, vError, pDiffusionErrors[index + 2 + delta]);          \
            ...
            target =                                                                                         \
              XMVectorMax((clampzero) ? g_XMZero : (XMVectorAdd(XMVectorNegate(scalev), g_XMOne)), target);  \
                                                                                                             \
            XMFLOAT4A tmp;                                                                                   \
...
_Use_decl_annotations_ bool DirectX::Internal::StoreScanlineDither(void* pDestination, size_t size,
                                                                   DXGI_FORMAT format, XMVECTOR* pSource,
                                                                   size_t count, float threshold, size_t y,
                                                                   size_t z,
                                                                   XMVECTOR* pDiffusionErrors) noexcept
{
    ...
    else
    {
        ...
        const XMVECTOR dither =
          XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(g_Dither + (z & 3) + ((y & 3) * 8)));
        ...
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                if (dPtr >= ePtr)
                    break;
                *dPtr =
                  (static_cast<uint32_t>(tmp.x) & 0xFFFFFF) | ((static_cast<uint32_t>(tmp.y) & 0xFF) << 24);
            }
            return true;
        }
        return false;
                             ...
                             _Out_ WICPixelFormatGUID& pfGUID, _Out_ WICPixelFormatGUID& targetGUID) noexcept
{
    ...
    if (sformat == DXGI_FORMAT_R16G16B16A16_FLOAT || sformat == DXGI_FORMAT_R16_FLOAT ||
        tformat == DXGI_FORMAT_R16G16B16A16_FLOAT || tformat == DXGI_FORMAT_R16_FLOAT)
    {
...
//-------------------------------------------------------------------------------------
HRESULT ConvertUsingWIC(_In_ const Image& srcImage, _In_ const WICPixelFormatGUID& pfGUID,
                        _In_ const WICPixelFormatGUID& targetGUID, _In_ TEX_FILTER_FLAGS filter,
                        _In_ float threshold, _In_ const Image& destImage)
{
#ifndef _WIN32
    ...
    hr = pWIC->CreateBitmapFromMemory(static_cast<UINT>(srcImage.width), static_cast<UINT>(srcImage.height),
                                      pfGUID, static_cast<UINT>(srcImage.rowPitch),
                                      static_cast<UINT>(srcImage.slicePitch), srcImage.pixels,
                                      source.GetAddressOf());
    if (FAILED(hr))
        return hr;
    ...
    hr = FC->CopyPixels(nullptr, static_cast<UINT>(destImage.rowPitch),
                        static_cast<UINT>(destImage.slicePitch), destImage.pixels);
    if (FAILED(hr))
        return hr;
...
//-------------------------------------------------------------------------------------
HRESULT ConvertCustom(_In_ const Image& srcImage, _In_ TEX_FILTER_FLAGS filter, _In_ const Image& destImage,
                      _In_ float threshold, size_t z) noexcept
{
    assert(srcImage.width == destImage.width);
    assert(srcImage.height == destImage.height);
    ...
    if (filter & TEX_FILTER_DITHER_DIFFUSION)
    {
        ...
        for (size_t h = 0; h < srcImage.height; ++h)
        {
            ...
            if (! StoreScanlineDither(pDest, destImage.rowPitch, destImage.format, scanline.get(), width,
                                      threshold, h, z, pDiffusionErrors))
                return E_FAIL;
    ...
    else
    {
        ...
        else
        {
            ...
            for (size_t h = 0; h < srcImage.height; ++h)
            {
                ...
                if (! StoreScanline(pDest, destImage.rowPitch, destImage.format, scanline.get(), width,
                                    threshold))
                    return E_FAIL;
...
// Convert image
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Convert(const Image& srcImage, DXGI_FORMAT format,
                                                TEX_FILTER_FLAGS filter, float threshold,
                                                ScratchImage& image) noexcept
{
    ...
    if (IsCompressed(srcImage.format) || IsCompressed(format) || IsPlanar(srcImage.format) ||
        IsPlanar(format) || IsPalettized(srcImage.format) || IsPalettized(format) ||
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Convert(const Image* srcImages, size_t nimages,
                                                const TexMetadata& metadata, DXGI_FORMAT format,
                                                TEX_FILTER_FLAGS filter, float threshold,
                                                ScratchImage& result) noexcept
{
    ...
    if (IsCompressed(metadata.format) || IsCompressed(format) || IsPlanar(metadata.format) ||
        IsPlanar(format) || IsPalettized(metadata.format) || IsPalettized(format) ||
    ...
    WICPixelFormatGUID pfGUID, targetGUID;
    const bool usewic =
      ! metadata.IsPMAlpha() && UseWICConversion(filter, metadata.format, format, pfGUID, targetGUID);
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::ConvertToSinglePlane(const Image* srcImages, size_t nimages,
                                                             const TexMetadata& metadata,
                                                             ScratchImage& result) noexcept
{
