...
constexpr WICConvert g_WICConvert[] = {
  ...
  {GUID_WICPixelFormat1bppIndexed, GUID_WICPixelFormat32bppRGBA,
   TEX_ALPHA_MODE_UNKNOWN}, // DXGI_FORMAT_R8G8B8A8_UNORM
  ...
  {GUID_WICPixelFormat64bppBGRA, GUID_WICPixelFormat64bppRGBA,
   TEX_ALPHA_MODE_UNKNOWN}, // DXGI_FORMAT_R16G16B16A16_UNORM
  ...
  {GUID_WICPixelFormat48bppRGBFixedPoint, GUID_WICPixelFormat64bppRGBAHalf,
   TEX_ALPHA_MODE_OPAQUE}, // DXGI_FORMAT_R16G16B16A16_FLOAT
  ...
  {GUID_WICPixelFormat64bppRGBAFixedPoint, GUID_WICPixelFormat64bppRGBAHalf,
   TEX_ALPHA_MODE_UNKNOWN}, // DXGI_FORMAT_R16G16B16A16_FLOAT
  ...
  {GUID_WICPixelFormat128bppPRGBAFloat, GUID_WICPixelFormat128bppRGBAFloat,
   TEX_ALPHA_MODE_UNKNOWN}, // DXGI_FORMAT_R32G32B32A32_FLOAT
  {GUID_WICPixelFormat128bppRGBFloat, GUID_WICPixelFormat128bppRGBAFloat,
   TEX_ALPHA_MODE_OPAQUE}, // DXGI_FORMAT_R32G32B32A32_FLOAT
...
//-------------------------------------------------------------------------------------
DXGI_FORMAT DetermineFormat(_In_ const WICPixelFormatGUID& pixelFormat, WIC_FLAGS flags, bool iswic2,
                            _Out_opt_ WICPixelFormatGUID* pConvert, _Out_ TEX_ALPHA_MODE* alphaMode) noexcept
{
    if (pConvert)
    ...
    *alphaMode = TEX_ALPHA_MODE_UNKNOWN;
    ...
    if (format == DXGI_FORMAT_UNKNOWN)
    {
        if (memcmp(&GUID_WICPixelFormat96bppRGBFixedPoint, &pixelFormat, sizeof(WICPixelFormatGUID)) == 0)
        {
            ...
            {
                if (pConvert)
                    memcpy_s(pConvert, sizeof(WICPixelFormatGUID), &GUID_WICPixelFormat96bppRGBFloat,
                             sizeof(GUID));
            ...
            {
                if (pConvert)
                    memcpy_s(pConvert, sizeof(WICPixelFormatGUID), &GUID_WICPixelFormat128bppRGBAFloat,
                             sizeof(GUID));
    ...
    case DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM:
        if (flags & WIC_FLAGS_NO_X2_BIAS)
        {
            ...
            if (pConvert)
                memcpy_s(pConvert, sizeof(WICPixelFormatGUID), &GUID_WICPixelFormat32bppRGBA1010102,
                         sizeof(GUID));
        }
        break;
...
//-------------------------------------------------------------------------------------
HRESULT DecodeMetadata(WIC_FLAGS flags, bool iswic2, _In_ IWICBitmapDecoder* decoder,
                       _In_ IWICBitmapFrameDecode* frame, _Out_ TexMetadata& metadata,
                       ...
                       _In_opt_ std::function<void(IWICMetadataQueryReader*)> getMQR)
{
    ...
    if (! (flags & WIC_FLAGS_IGNORE_SRGB))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                if (SUCCEEDED(metareader->GetMetadataByName(L"/sRGB/RenderingIntent", &value)) &&
                    value.vt == VT_UI1)
                {
                    sRGB = true;
                }
                else if (SUCCEEDED(metareader->GetMetadataByName(L"/gAMA/ImageGamma", &value)) &&
                         value.vt == VT_UI4)
                {
            ...
            {
                if (SUCCEEDED(metareader->GetMetadataByName(L"/app1/ifd/exif/{ushort=40961}", &value)) &&
                    value.vt == VT_UI2)
                {
                    sRGB = (value.uiVal == 1);
                }
            ...
            {
                if (SUCCEEDED(metareader->GetMetadataByName(L"/ifd/exif/{ushort=40961}", &value)) &&
                    value.vt == VT_UI2)
                {
                    sRGB = (value.uiVal == 1);
                }
...
#else
            else if (SUCCEEDED(metareader->GetMetadataByName(L"System.Image.ColorSpace", &value)) &&
                     value.vt == VT_UI2)
            {
                sRGB = (value.uiVal == 1);
            }
...
//-------------------------------------------------------------------------------------
HRESULT DecodeSingleFrame(WIC_FLAGS flags, const TexMetadata& metadata, const WICPixelFormatGUID& convertGUID,
                          _In_ IWICBitmapFrameDecode* frame, _Inout_ ScratchImage& image)
{
    ...
    {
        hr = frame->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch), static_cast<UINT>(img->slicePitch),
                               img->pixels);
        if (FAILED(hr))
            return hr;
    }
    else
    {
        ...
        hr =
          FC->Initialize(frame, convertGUID, GetWICDither(flags), nullptr, 0, WICBitmapPaletteTypeMedianCut);
        if (FAILED(hr))
            return hr;

        hr = FC->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch), static_cast<UINT>(img->slicePitch),
                            img->pixels);
        if (FAILED(hr))
            return hr;
    }
...
//-------------------------------------------------------------------------------------
HRESULT DecodeMultiframe(WIC_FLAGS flags, const TexMetadata& metadata, _In_ IWICBitmapDecoder* decoder,
                         _Inout_ ScratchImage& image)
{
    ...
    for (size_t index = 0; index < metadata.arraySize; ++index)
    {
        ...
        if (w == metadata.width && h == metadata.height)
        {
            ...
            {
                hr = frame->CopyPixels(nullptr, static_cast<UINT>(img->rowPitch),
            ...
            else
            {
                ...
                hr = FC->Initialize(frame.Get(), sourceGUID, GetWICDither(flags), nullptr, 0,
        ...
        else
        {
            ...
            hr = scaler->Initialize(frame.Get(), static_cast<UINT>(metadata.width),
                                    static_cast<UINT>(metadata.height), GetWICInterp(flags));
            if (FAILED(hr))
                return hr;
            ...
            else
            {
                ...
                hr = FC->Initialize(scaler.Get(), sourceGUID, GetWICDither(flags), nullptr, 0,
                                    WICBitmapPaletteTypeMedianCut);
                if (FAILED(hr))
                    return hr;
...
//-------------------------------------------------------------------------------------
HRESULT EncodeMetadata(WIC_FLAGS flags, _In_ IWICBitmapFrameEncode* frame, const GUID& containerFormat,
                       DXGI_FORMAT format)
{
    if (! frame)
        return E_POINTER;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        const bool sRGB =
          ((flags & WIC_FLAGS_FORCE_LINEAR) == 0) && ((flags & WIC_FLAGS_FORCE_SRGB) != 0 || IsSRGB(format));
...
//-------------------------------------------------------------------------------------
HRESULT EncodeImage(const Image& image, WIC_FLAGS flags, _In_ REFGUID containerFormat,
                    _In_ IWICBitmapFrameEncode* frame, _In_opt_ IPropertyBag2* props,
                    _In_opt_ const GUID* targetFormat)
{
    ...
    if (memcmp(&targetGuid, &pfGuid, sizeof(WICPixelFormatGUID)) != 0)
    {
        ...
        hr = pWIC->CreateBitmapFromMemory(static_cast<UINT>(image.width), static_cast<UINT>(image.height),
                                          pfGuid, static_cast<UINT>(image.rowPitch),
                                          static_cast<UINT>(image.slicePitch), image.pixels,
                                          source.GetAddressOf());
        if (FAILED(hr))
            return hr;
        ...
        hr = FC->Initialize(source.Get(), targetGuid, GetWICDither(flags), nullptr, 0,
                            WICBitmapPaletteTypeMedianCut);
        if (FAILED(hr))
            return hr;
    ...
    else
    {
        ...
        hr =
          frame->WritePixels(static_cast<UINT>(image.height), static_cast<UINT>(image.rowPitch),
                             static_cast<UINT>(image.slicePitch), reinterpret_cast<uint8_t*>(image.pixels));
        if (FAILED(hr))
            return hr;
    }
...
//-------------------------------------------------------------------------------------
HRESULT EncodeMultiframe(_In_reads_(nimages) const Image* images, size_t nimages, WIC_FLAGS flags,
...
DirectX::GetMetadataFromWICMemory(const void* pSource, size_t size, WIC_FLAGS flags, TexMetadata& metadata,
                                  std::function<void(IWICMetadataQueryReader*)> getMQR)
{
    ...
    hr =
      stream->InitializeFromMemory(static_cast<BYTE*>(const_cast<void*>(pSource)), static_cast<UINT>(size));
    if (FAILED(hr))
        return hr;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
DirectX::GetMetadataFromWICFile(const wchar_t* szFile, WIC_FLAGS flags, TexMetadata& metadata,
                                std::function<void(IWICMetadataQueryReader*)> getMQR)
{
    if (! szFile)
        return E_INVALIDARG;
    ...
    // Initialize WIC
    ComPtr<IWICBitmapDecoder> decoder;
    HRESULT hr = pWIC->CreateDecoderFromFilename(szFile, nullptr, GENERIC_READ,
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
DirectX::LoadFromWICMemory(const void* pSource, size_t size, WIC_FLAGS flags, TexMetadata* metadata,
                           ScratchImage& image, std::function<void(IWICMetadataQueryReader*)> getMQR)
{
    if (! pSource || size == 0)
        return E_INVALIDARG;
    ...
    hr = stream->InitializeFromMemory(static_cast<uint8_t*>(const_cast<void*>(pSource)),
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveToWICMemory(const Image& image, WIC_FLAGS flags,
                                                        REFGUID containerFormat, Blob& blob,
...
_Use_decl_annotations_ HRESULT DirectX::SaveToWICMemory(const Image* images, size_t nimages, WIC_FLAGS flags,
                                                        REFGUID containerFormat, Blob& blob,
                                                        const GUID* targetFormat,
                                                        std::function<void(IPropertyBag2*)> setCustomProps)
{
    ...
    if (nimages > 1)
        hr = EncodeMultiframe(images, nimages, flags, containerFormat, stream.Get(), targetFormat,
                              setCustomProps);
    else
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveToWICFile(const Image& image, WIC_FLAGS flags,
                                                      REFGUID containerFormat, const wchar_t* szFile,
                                                      const GUID* targetFormat,
                                                      std::function<void(IPropertyBag2*)> setCustomProps)
{
    if (! szFile)
...
_Use_decl_annotations_ HRESULT DirectX::SaveToWICFile(const Image* images, size_t nimages, WIC_FLAGS flags,
                                                      REFGUID containerFormat, const wchar_t* szFile,
                                                      const GUID* targetFormat,
                                                      std::function<void(IPropertyBag2*)> setCustomProps)
{
